...
class ConvTokenizer(nn.Module):
    def __init__(self, embedding_dim=64):
        ...
        # self.block = VGG(pretrained=True)
...
class ConvStage(Module):
    def __init__(self,
        ...
        self.downsample = Conv2d(embedding_dim_in,
                                 ...
                                 stride=(2, 2),
                                 padding=(1, 1))
        # self.dconv1 = nn.Conv2d(96,96,3,1, 1, dilation=1)
        # self.dconv2 = nn.Conv2d(96, 96, 3, 1, 2, dilation=2)
        # self.dconv3 = nn.Conv2d(96, 96, 3, 1, 3,dilation=3)
    def forward(self, x):
...
class ConvMLP(nn.Module):
    def __init__(self,
        ...
        for i in range(0, len(blocks)):
            ...
            if i == len(blocks)-1:
                stages1 =  BasicStage(num_blocks=blocks[i],
                               embedding_dims=dims[i:i + 2],
                               mlp_ratio=mlp_ratios[i],
                               stochastic_depth_rate=0.1,
                               downsample=(i + 1 < len(blocks)))
                self.stages1.append(stages1)
    ...
    def forward(self, x):
        # print("x_original:",x.shape)
        x = self.tokenizer(x)
        # print("after1:",x.shape)
